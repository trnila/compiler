...
public class NicePrintingVisitor implements IRVisitor {
    ...
    public void visit(ForStatement st) {
        ...
        String afterthought = data.pop();
        String condition = data.pop();
	    String initialization = data.pop();

        String result = "for(" + initialization + "; " + condition + "; " + afterthought + ") " + body;
        data.push(result);
    }
    ...
    public void visit(VariableDeclaration decl) {
        StringBuilder sb = new StringBuilder(";");
        for (int i = 1; i < decl.getVariables().size(); i++) {
            sb.insert(0, "," + data.pop());
        }
        sb.insert(0, decl.getType() + " " + data.pop());
        data.push(sb.toString());
    }
}
